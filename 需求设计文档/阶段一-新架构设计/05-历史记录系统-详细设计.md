...
适用于 **PCB 图形编辑器** 场景，需处理大量微小变更（如 Footprint 拖拽、交互式布线）带来的性能与合并问题。
...
export interface ICommand {
  ...
  // 命令名称 (用于 UI 显示，如 "Move Footprint")
...
用于组合多个原子命令（如：删除一个 Footprint 同时删除其关联的走线与过孔）。
...
当删除一个 Footprint 时，需要同时删除与其相连的 Tracks/Vias。
**策略**:
1.  用户点击删除 Footprint。
2.  Service 层计算依赖关系：找到与该 Footprint 关联的 TrackIds/ViaIds。
3.  `history.startTransaction('Delete Footprint With Connections')`。
4.  生成多个 `DeleteTrackCommand`（针对所有相关 Tracks）。
5.  生成多个 `DeleteViaCommand`（针对所有相关 Vias）。
6.  生成 `DeleteFootprintCommand`。
7.  `history.commitTransaction()`。
这样撤销时，会一次性恢复 Footprint 及其连接走线与过孔。
...
- 图形开发专家（graphics-engineer）
  - 负责 ICommand、HistoryManager 以及 PCB 场景下各类具体命令（PlaceFootprint/RouteTrack/PlaceVia/MoveEntities/PropertyChange 等）的实现；
  - 与 UI、工具系统对接，定义何处生成命令、如何使用事务和合并策略。
- 前端架构（frontend-architect）
  - 将撤销/重做状态（canUndo/canRedo、最近命令名称等）接入工具栏、快捷键和右键菜单；
  - 确保 UI 层的操作全部通过命令通路修改内核状态。
- 产品架构师（product-architect）
  - 定义用户视角下需要被记录的操作粒度（例如一次交互式布线在历史中体现为几步）；
  - 为复杂操作（批量移动、多实体编辑等）的历史表现给出产品规范。
- 测试专家（qa-automation-expert）
  - 设计围绕历史记录系统的自动化测试：连续拖拽、批量删除、撤销/重做边界情况等；
  - 关注命令合并、事务提交/回滚对 PCB 实体数据一致性的影响。
